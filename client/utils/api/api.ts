...
const apiService = {
  ...
  getGradJobDetailReview: async ({ queryKey }: any) => {
    const id = queryKey[1].id
    try {
      const data = await axios.get(`${DEFAULT_BASE_URL}/grad-job-reviews/${id}`)
      return data.data.data
    } catch (error) {
      console.error(error)
    }
  ...
  createGradJobReview: async (body: ICreateGradJobReviewReq) => {
    const res = await axios.post(`${DEFAULT_BASE_URL}/grad-job-reviews`, body)
    return res
  },
  ...
  voteInterviewReview: async (param: IVoteReq) => {
    const { value, id } = param
    try {
      await axios.post(`${DEFAULT_BASE_URL}/interview-reviews/${id}/votes`, {
        value
      })
    } catch (error) {
      throw error
